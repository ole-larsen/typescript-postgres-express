...
2add root user DEFAULT_USER_PASSWORD=emptypassword
12. add root user DEFAULT_USER_EMAIL=ole.larssen777@gmail.com
13. add provider url PROVIDER_URL=http://provider:5551
...
# give the storage correct read/write permissions
sudo chmod 777 /data/storage

# starting from macOS 10.15 even the admin cannot create directory at root
# so lets create the storage directory under the home directory.
mkdir -p ~/data/storage
# user account has automatically read and write permissions for ~/data/storage.
```
- Start your mongoDB server (you'll probably want another command prompt)
```bash
mongod

# on macOS 10.15 or above the storage directory is under home directory
mongod --dbpath ~/data/storage
```
